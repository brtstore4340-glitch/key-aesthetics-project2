...
export default function CreateOrder() {
  const { user } = useAuth();
  // Ensure we have a user, though protected route should handle this.
  // Using user directly instead of state copy to keep it in sync.
  ...
  // Removed sessionStorage useEffect as useAuth handles persistence.
  ...
  if (!currentUser) return <div className="p-8 text-white">Loading user...</div>;
  ...
  const handleSubmitOrder = () => {
    const orderData = {
      ...
      sales_rep_name: currentUser.name || currentUser.username // Fallback to username if name is missing
    };
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {products.map((product) => (
                  ...
                  >
                    ...
                    <p className="text-amber-300 font-light mt-1">฿{Number(product.price).toLocaleString()}</p>
        ...
        {step === 5 && (
          ...
          >
            ...
            <div className="flex flex-col gap-4">
              ...
              <Link href="/orders">
                <GlassButton variant="gold" className="w-full">View My Orders</GlassButton>
              </Link>
            </div>
          </motion.div>
        )}
